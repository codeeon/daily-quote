...
const API_BASE_URL = import.meta.env.DEV 
  ? 'http://localhost:3001/api/advice' // 로컬에서는 프록시 서버 사용
  : '/api/advice'; // 프로덕션에서는 서버 액션 사용
...
export class ApiService {
  ...
  async fetchQuoteFromApi(): Promise<Quote> {
    ...
    try {
      ...
      console.log(`API 호출 성공 [${import.meta.env.DEV ? '로컬 프록시' : '서버 액션'}]:`, data.fallback ? '(fallback)' : '(실제 API)');
